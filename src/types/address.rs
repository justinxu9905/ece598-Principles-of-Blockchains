...
impl Address {
    pub fn from_public_key_bytes(bytes: &[u8]) -> Address {
        let hashed0 = digest::digest(&digest::SHA256, bytes);
        let hashed1 = hashed0.as_ref();
        let mut re = [0u8; 20];
        for i in 1..21 {
            re[i-1] = hashed1[hashed1.len()-21+i];
        }
        return Address(re);
    }

    pub fn generate_random_address() -> Address {
        let rand_addr: [u8; 20] = rand::random();
        return Address(rand_addr);
    }
}
